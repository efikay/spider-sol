...
impl GameTableau {
    ...
    fn calculate_card_on_card_moves(&self) -> Vec<AvailableMove> {
        ...
        for (src_pile_idx, src_pile) in self.piles.iter().enumerate() {
            for dest_pile_idx in 0..PILES_AMOUNT {
                if dest_pile_idx == src_pile_idx {
                    continue;
                }

                let dest_pile = &self.piles[dest_pile_idx];

                if let Some(dest_tip_card) = dest_pile.playable_cards().last() {
                    for (src_card_idx, src_card) in
                        src_pile.playable_cards().iter().rev().enumerate()
                    {
    ...
    fn calculate_empty_pile_moves(&self) -> Vec<AvailableMove> {
        ...
        for (src_pile_idx, src_pile) in self.piles.iter().enumerate() {
            for dest_pile_idx in 0..PILES_AMOUNT {
                ...
                for (src_card_idx, _) in src_pile.playable_cards().iter().enumerate() {
                    available_moves.push(AvailableMove::new(
                        (src_pile_idx, src_card_idx),
                        dest_pile_idx,
                    ));
